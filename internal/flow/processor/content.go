...
func (p *ContentRequestProcessor) ProcessRequest(
	...
	if include != IncludeContentsNone && invocation.Session != nil {
		...
		log.Infof("summaryUpdatedAt: %v, len(messages): %d", summaryUpdatedAt, len(messages))
	}
	log.Infof("effective include: %s, invocation.Message.Content: %s, needToAddInvocationMessage: %v", include, invocation.Message.Content, needToAddInvocationMessage)
