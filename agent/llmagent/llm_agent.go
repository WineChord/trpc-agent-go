...
func buildRequestProcessors(name string, options *Options) []flow.RequestProcessor {
	...
	// Runtime include mode (include_contents) is now honored at request time
	// inside ContentRequestProcessor by reading invocation.RunOptions.
	contentProcessor := processor.NewContentRequestProcessor(
		processor.WithIncludeContents(processor.IncludeContentsFiltered),
