...
const finalModelCallNotice = "[OpenClaw Budget Notice] This is the " +
	...
	"future tool use. Do not ask to continue or promise future execution; " +
	"answer now with the best supported final value. Put the answer in " +
	"visible assistant message content for the user, not only in internal " +
	"reasoning or thinking content. If evidence is incomplete, give the " +
	"best supported final answer now. If the original task requires a " +
	"final-answer format or marker such as FINAL ANSWER:, follow it exactly " +
	"and avoid extra explanation."
