...
const (
	...
	flagDeadlineFinalizationApproxRunesPerToken       = "deadline-finalization-approx-runes-per-token"
...
type runOptions struct {
	...
	DeadlineFinalizationApproxRunesPerToken       float64
...
func parseRunOptions(args []string) (runOptions, error) {
	...
	fs.Float64Var(
		&opts.DeadlineFinalizationApproxRunesPerToken,
		flagDeadlineFinalizationApproxRunesPerToken,
		0,
		"Approximate runes per token for deadline finalization trimming "+
...
type agentRunConfig struct {
	...
	DeadlineFinalizationApproxRunesPerToken *float64 `yaml:"deadline_finalization_approx_runes_per_token,omitempty"`
...
func (cfg *fileConfig) apply(
	...
	if cfg.Agent != nil {
		...
		if cfg.Agent.DeadlineFinalizationApproxRunesPerToken != nil &&
			!flagWasSet(set, flagDeadlineFinalizationApproxRunesPerToken) {
			v := *cfg.Agent.DeadlineFinalizationApproxRunesPerToken
			...
			opts.DeadlineFinalizationApproxRunesPerToken = v
		}
...
func finalizeRunOptions(opts *runOptions) error {
	...
	if v := opts.DeadlineFinalizationApproxRunesPerToken; math.IsNaN(v) ||
		math.IsInf(v, 0) || v < 0 {
		return fmt.Errorf(
			"invalid deadline finalization approx runes per token: %v",
			v,
		)
	}
